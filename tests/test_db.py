...
class JSONFieldTests(_AnySequenceTestCase):
    ...
    def test_uuid_serialization(self):
        # orjson serializes UUIDs natively; the round-trip produces the string form
        value = {"uuid": uuid.UUID("d85e2076-b67c-4ee7-8c3a-2bf5a2cc2475")}
        obj = NullableJSONModel(value=value)
        obj.clean_fields()
        obj.save()
        obj.refresh_from_db()
        self.assertEqual(obj.value, {"uuid": "d85e2076-b67c-4ee7-8c3a-2bf5a2cc2475"})

    def test_db_check_constraints(self):
        with (
            mock.patch(
                "django_orjson.db.orjson.dumps", return_value=b"{@!invalid json}"
            ),
...
class TestMethods(SimpleTestCase):
    def test_deconstruct(self):
        ...
        self.assertEqual(path, "django_orjson.db.JSONField")
        self.assertEqual(args, [])
        self.assertEqual(kwargs, {})

    def test_deconstruct_with_option(self):
        field = JSONField(option=orjson.OPT_SORT_KEYS)
        name, path, args, kwargs = field.deconstruct()
        self.assertEqual(path, "django_orjson.db.JSONField")
        self.assertEqual(kwargs["option"], orjson.OPT_SORT_KEYS)
        self.assertNotIn("encoder", kwargs)
        self.assertNotIn("decoder", kwargs)
    ...
    def test_get_prep_value(self):
        class JSONFieldGetPrepValue(JSONField):
        ...
        # prepared=False: get_prep_value is called first, transforming True
        self.assertEqual(
            field.get_db_prep_value(True, connection, prepared=False),
            '{"value":true}',
        )
        # prepared=True: get_prep_value is skipped, orjson encodes the raw value
        self.assertEqual(
            field.get_db_prep_value(True, connection, prepared=True),
            "true",
        )
...
class TestValidation(SimpleTestCase):
    def test_encoder_not_supported(self):
        with pytest.raises(TypeError, match="encoder is not supported for JSONField"):
            JSONField(encoder=object)

    def test_decoder_not_supported(self):
        with pytest.raises(TypeError, match="decoder is not supported for JSONField"):
            JSONField(decoder=object)

    def test_validation_error(self):
        field = JSONField()
        msg = "Value must be valid JSON."
        with self.assertRaisesMessage(ValidationError, msg):
            field.clean({"obj": object()}, None)

    def test_uuid_validates(self):
        # orjson serializes UUID natively; no custom encoder required
        field = JSONField()
...
class TestSerialization(SimpleTestCase):
    ...
    def test_xml_serialization(self):
        test_xml_data = (
            ...
            '<object model="testapp.nullablejsonmodel">'
...
class TestSaveLoad(_AnySequenceTestCase):
    ...
    def test_list(self):
        values = [[], ["John", 20, 180.3], [True, [False, None]]]
        for value in values:
            with self.subTest(value=value):
                obj = JSONModel.objects.create(value=value)
                obj.refresh_from_db()
...
class TestQuerying(_AnySequenceTestCase):
    ...
    def test_lookups_special_chars(self):
        ...
        json_value = dict.fromkeys(test_keys, "some value")
    ...
    def test_lookups_special_chars_double_quotes(self):
        test_keys = [
            'double"',
            "m\\i@x. m\U0001f921'a,t{{{ch}}}e?d$\"'es\uffff'ca\uffff'pe",
        ]
        json_value = dict.fromkeys(test_keys, "some value")
        obj = NullableJSONModel.objects.create(value=json_value)
        obj.refresh_from_db()
    ...
    def test_contains_contained_by_with_key_transform(self):
        tests = [
            ...
            (
                ...
                KeyTransform(
                    "x",
                    RawSQL(self.raw_sql, ['{"x": {"a": "b", "c": 1, "d": "e"}}']),
                ),
            ),
        ]
